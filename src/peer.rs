...
struct Peer {
    ...
    /// Active bindings (announced port -> live listener)
...
/// A live local listener forwarding to one of a peer's announced ports.
struct Binding {
    /// The port actually listening locally. Differs from the announced port
    /// when remapped by `bind`, or when the OS picked one.
    local: u16,
...
pub struct PeerManager {
    ...
    pins: Pins,
    /// What each exposed port is backed by, including the default forward
    services: Arc<Services>,
    /// Announced port -> local port to bind it on. A port with no entry binds
    /// on itself; 0 asks the OS for any free port. Exists because the peer
    /// chooses the number and the number may already be taken here.
    binds: DashMap<u16, u16>,
...
impl PeerManager {
        ...
        loop {
            tokio::select! {
                ...
                result = conn.accept_bi() => {
                    ...
                    let peer = peer.clone();
                    tokio::spawn(async move {
                        if let Err(e) = Self::handle_bi_stream(&grants, &services, &peer, send, recv).await {
    ...
    async fn handle_bi_stream(
        ...
        // The grant is the only gate, whichever way the port is backed.
        ...
        services.serve(port, peer.endpoint_id, send, recv).await
    }
    ...
    async fn update_peer_ports(self: &Arc<Self>, peer: &Arc<Peer>, new_ports: Vec<u16>) {
        ...
        for &port in &new_ports {
            ...
            if let Some(holder) = self.find_holder(self.local_for(port), &peer.endpoint_id) {
            ...
            self.create_binding(peer, port).await;
        }

        *peer.exposed_ports.write().await = new_ports;
    }

    /// Bind one announced port locally. Bind before recording anything: the
    /// listener is live from here, and a failed bind must not leave a phantom
    /// entry, since a port with no binding is retried on the next announce.
    async fn create_binding(&self, peer: &Arc<Peer>, port: u16) {
        if let Err(e) = self.try_create_binding(peer, port).await {
            error!("binding port {} failed: {}", port, e);
        }
    }

    async fn try_create_binding(&self, peer: &Arc<Peer>, port: u16) -> Result<()> {
        let listener = tunnel::bind_listener(self.local_for(port)).await?;
        ...
        peer.bindings.insert(port, Binding { local, handle });
        info!("created binding for port {} on local port {}", port, local);
        Ok(())
    }

    /// Remap an announced port to a different local port, or clear the remap
    /// with `local: None`. Takes effect immediately: any live binding for the
    /// port is torn down and rebuilt, so a busy port can be moved without
    /// waiting for the peer to re-announce.
    pub async fn set_bind(&self, port: u16, local: Option<u16>) {
        let _guard = self.bind_lock.lock().await;

        match local {
            Some(local) => {
                self.binds.insert(port, local);
            }
            None => {
                self.binds.remove(&port);
            }
        }
        ...
        for peer in &peers {
            if peer.exposed_ports.read().await.contains(&port) {
                self.create_binding(peer, port).await;
            }
        }
    }
    ...
    /// Another peer (not `exclude`) whose binding already occupies local port
    /// `local`.
    ///
    /// Keyed on the local port rather than the announced one: `bind` broke the
    /// identity between the two, so two peers announcing the same port may now
    /// land on different listeners (no collision), and two different announced
    /// ports may be remapped onto the same one (a collision the announced-port
    /// view cannot see). A local port of 0 asks the OS for a free one, which
    /// cannot collide by construction.
    fn find_holder(&self, local: u16, exclude: &EndpointId) -> Option<Arc<Peer>> {
        if local == 0 {
            return None;
        }
        self.peers
            .iter()
            .find(|e| {
                e.key() != exclude && e.value().bindings.iter().any(|b| b.value().local == local)
            })
            .map(|e| e.value().clone())
    }
    ...
    async fn release_binding(peer: &Arc<Peer>, port: u16) {
        if let Some((_, binding)) = peer.bindings.remove(&port) {
    ...
    async fn handle_enrollment(self: &Arc<Self>, conn: Connection) -> Result<Option<Arc<Peer>>> {
        ...
        if let Some(ports) = &early_ports {
            for &port in ports {
                if let Some(holder) = self.find_holder(self.local_for(port), &remote_id) {
                    if Self::probe_peer(&holder).await {
                        error!(
                            "rejected incoming peer {}: port {} already held by connected peer {} (\"{}\")",
                            remote_id,
